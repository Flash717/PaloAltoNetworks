...
def space_used(basedir: Dict) -> int:
    ...
    result = 0
    for k in basedir:
        if isinstance(basedir[k], dict):
            result += space_used(basedir[k])
        else:
            result += len(basedir[k])
    return result
...
def space_used_by_filetype(basedir: Dict) -> Dict[str, int]:
    ...
    result = {}
    for i in basedir:
        if isinstance(basedir[i], dict):
            new_result = space_used_by_filetype(basedir[i])
            result = {k: result.get(k, 0) + new_result.get(k, 0) for k in set(result) | set(new_result) }
        else:
            ft = filetype(i)
            result[ft] = result.get(ft, 0) + len(basedir[i])
    return result
...
def largest_space_used_by_filetype(basedir: Dict) -> Dict[str, Dict]:
    ...
    for i in basedir:
        if isinstance(basedir[i], dict):
            new_result = largest_space_used_by_filetype(basedir[i])
            for j in new_result:
                if j in result:
                    # compare size and update
                    if new_result[j]['space'] > result[j]['space']:
                        result[j] = {'name': i + '/' + new_result[j]
                                     ['name'], 'space': new_result[j]['space']}
                else:
                    result[j] = {'name': i + '/' + new_result[j]
                                 ['name'], 'space': new_result[j]['space']}
        else:
            ft = filetype(i)
            space = len(basedir[i])
            if ft not in result or result[ft]['space'] < space:
                result[ft] = {'name': i, 'space': space}
...
def files_greater_than(basedir: Dict, threshold: int) -> List[str]:
    ...
    for i in basedir:
        if isinstance(basedir[i], dict):
            new_result = files_greater_than(basedir[i], threshold)
            new_result = [i + "/" + j for j in new_result]
            result.extend(new_result)
        else:
            if len(basedir[i]) > threshold:
                result.append(i)
    return result
...
def delete_files_greater_than(basedir: Dict, threshold: int) -> int:
    ...
    result = 0
    for i in list(basedir):
        # need to use list here so we can delete items that exceed threshold
        if isinstance(basedir[i], dict):
            result += delete_files_greater_than(basedir[i], threshold)
        else:
            space = len(basedir[i])
            if space > threshold:
                result += space
